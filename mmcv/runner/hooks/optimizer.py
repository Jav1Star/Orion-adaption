...
if (digit_version(TORCH_VERSION) >= digit_version('1.6.0')):
    ...
    class Fp16OptimizerHook(OptimizerHook):
        ...
        def after_train_iter(self, runner):
            ...
            #         if torch.any(torch.isnan(para.grad)):
            #             print(name)
            #             nan_name_list.append(name)
            #             print(para.grad)
            #         if torch.any(torch.isinf(para.grad)):
            #             print(name)
            #             inf_name_list.append(name)
            #             print(para.grad)
            # print("============= nan grad paramters ================")
            # for name in nan_name_list:
            #     print(name)
            # print("============= inf grad paramters ================")
            # for name in inf_name_list:
            #     print(name)
            # grad clip
            if self.grad_clip is not None:
                grad_norm = self.clip_grads(runner.model.parameters())
    ...
    def before_run(self, runner) -> None:
        ...
        for module_name, v in self.custom_fp16.items():
            if module_name == 'model':
                runner.model.module.fp16_enabled = v
            else:
