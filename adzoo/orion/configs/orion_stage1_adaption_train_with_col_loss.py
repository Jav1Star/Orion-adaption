...
num_epochs = 2
checkpoint_interval = 1500  # None 表示默认按每个 epoch 的 iter 数保存一次 checkpoint。
use_gen_token = True
use_col_loss = True
...
orion_adaption_cfg = dict(
    ...
    stage1_aux_div_enabled=True,
    # 关键调用点：156k resume 时再接入 loss_div，先用更宽松的 margin 和更小的权重，避免打断已有的规划/VLM 收敛态。
    stage1_aux_div_margin=0.10,
    stage1_aux_bal_epsilon={{_base_.orion_adaption_cfg.stage1_aux_bal_epsilon}},
    stage1_aux_div_ratio_start=0.002,
    stage1_aux_div_ratio_end=0.02,
    stage1_aux_div_warmup_steps=5000,
    stage1_aux_bal_ratio={{_base_.orion_adaption_cfg.stage1_aux_bal_ratio}},
    stage1_aux_div_queue_size=32,
